...
(async () => {
  ...
  for (const p of (pg.items || [])) {
    ...
    const prompt = `Você é redator especialista de um portal premium sobre inteligência artificial em português do Brasil. A página "${p.title}" de um site tem uma ferramenta interativa, mas precisa de um artigo editorial ÚNICO e profundo logo abaixo dela.\n\nEscreva um artigo de 800-1100 palavras sobre o tema da página "${p.title}": o que é, como funciona por dentro, como usar bem (passo a passo), exemplos práticos reais, boas práticas e erros comuns, e uma seção <h2>Perguntas Frequentes</h2> com 4 <h3>. Tom especialista, acessível e confiável. Não se identifique como IA. Não repita instruções óbvias de uso do widget. HTML puro com <h2>, <h3>, <p>, <ul><li> (sem h1, sem html/head/body, sem markdown). Comece direto com <h2>.`;
  ...
  for (const p of finos) {
    ...
    const prompt = `Você é redator especialista de um portal premium sobre inteligência artificial em português do Brasil. O post "${p.title}" está raso e precisa ser reescrito como matéria de jornal completa e profunda.\n\nConteúdo atual (use como base factual, NÃO invente fatos novos além dele):\n${p.content.replace(/<[^>]+>/g, " ").slice(0, 2500)}\n\nReescreva como artigo de 1300-1800 palavras: contexto do tema, análise aprofundada, o que isso significa na prática pro leitor brasileiro, exemplos, e uma seção <h2>Perguntas Frequentes</h2> com 4 <h3>. Tom jornalístico especialista. Não se identifique como IA. HTML puro com <h2>, <h3>, <p>, <ul><li> (sem h1, sem html/head/body, sem markdown). Comece direto com <p>.`;
    const art = limpa(await callAI(prompt));
