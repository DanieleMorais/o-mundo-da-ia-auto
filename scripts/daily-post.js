...
async function callAI(prompt) {
  ...
  if (OR) {
    for (const model of ["openai/gpt-4o-mini", "meta-llama/llama-3.3-70b-instruct:free"]) {
      for (let t = 0; t < 3; t++) {
        try {
          const r = await fetch("https://openrouter.ai/api/v1/chat/completions", { method: "POST", headers: { Authorization: "Bearer " + OR, "Content-Type": "application/json", "HTTP-Referer": "https://omundodaia.blogspot.com", "X-Title": "O Mundo da IA" }, body: JSON.stringify({ model, messages: [{ role: "user", content: prompt }], temperature: 0.8, max_tokens: 3500 }), signal: AbortSignal.timeout(90000) });
          if (r.status === 429 || r.status >= 500) { await sleep(12000); continue; }
          if (!r.ok) break;
