// Publica ATÉ 10 matérias com as últimas notícias de IA por execução (rodado de 4 em 4 horas).
// Credenciais vêm de secrets (env). Dedup pelo feed do blog — nunca repete.
...
const QUERIES = [
  "inteligência artificial", "ChatGPT OpenAI", "Google Gemini IA", "IA lançamento novidade",
  "ferramenta de inteligência artificial", "IA generativa imagem vídeo", "IA emprego trabalho tecnologia",
  "modelo de linguagem IA", "Anthropic Claude IA", "inteligência artificial Brasil", "agentes de IA", "IA robô automação",
];
const CATS = ["Modelos de IA", "Ferramentas de IA", "ChatGPT", "IA de Imagem", "IA de Vídeo", "IA nos Negócios", "Ciência & Pesquisa"];
...
async function existentes() {
  try { const r = await fetch(`https://www.blogger.com/feeds/${BLOG_ID}/posts/default?alt=json&max-results=500`); return (((await r.json()).feed || {}).entry || []).map(e => norm(e.title.$t)); } catch { return []; }
}
async function coletar() {
  const vistos = new Set(), lista = [];
  const qs = QUERIES.slice().sort(() => 0.5 - Math.random()); // varia a ordem a cada run
  for (const q of qs) {
    try {
      const url = `https://news.google.com/rss/search?q=${encodeURIComponent(q + " when:15d")}&hl=pt-BR&gl=BR&ceid=BR:pt-419`;
      ...
      for (const it of (xml.match(/<item>([\s\S]*?)<\/item>/g) || [])) {
        ...
        const fonte = g(/<source[^>]*>(.*?)<\/source>/);
        const k = norm(titulo).slice(0, 60);
        if (titulo.length > 25 && !vistos.has(k)) { vistos.add(k); lista.push({ titulo, fonte }); }
      }
    } catch {}
    await sleep(400);
  }
...
async function callAI(prompt) {
  if (OR_KEY) {
    for (const model of ["openai/gpt-4o-mini", "meta-llama/llama-3.3-70b-instruct:free"]) {
      for (let t = 0; t < 3; t++) {
        try {
          const r = await fetch("https://openrouter.ai/api/v1/chat/completions", { method: "POST", headers: { Authorization: "Bearer " + OR_KEY, "Content-Type": "application/json", "HTTP-Referer": "https://omundodaia.blogspot.com", "X-Title": "O Mundo da IA" }, body: JSON.stringify({ model, messages: [{ role: "user", content: prompt }], temperature: 0.8, max_tokens: 3000 }), signal: AbortSignal.timeout(90000) });
          if (r.status === 429 || r.status >= 500) { await sleep(10000); continue; }
          if (!r.ok) break;
          const c = (await r.json()).choices?.[0]?.message?.content?.trim();
  ...
  for (let t = 0; t < 3; t++) {
    try {
      const r = await fetch("https://text.pollinations.ai/openai", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ model: "openai-fast", messages: [{ role: "user", content: prompt }], temperature: 0.78 }), signal: AbortSignal.timeout(90000) });
...
async function gerar(n) {
  const prompt = `Você é redator(a) de um portal de notícias de Inteligência Artificial em português. Escreva uma MATÉRIA jornalística, clara e informativa (700-1100 palavras) inspirada nesta notícia recente:\n"${n.titulo}"${n.fonte ? " (fonte: " + n.fonte + ")" : ""}\n\nREGRAS: explique o contexto, o que significa e por que importa; atribua à fonte ("segundo a ${n.fonte || "imprensa especializada"}"). NÃO invente números, datas, preços ou declarações específicas que não estejam na manchete — se não tiver certeza, escreva de forma geral e explicativa. Tom acessível, use você. Estrutura: <p> de abertura, 4-6 <h2> com <h3>/<ul> quando útil, e uma seção final <h2>O que isso muda para você</h2>. NÃO copie a manchete literalmente no título. NÃO se identifique como IA. NÃO escreva html/head/body/h1 nem markdown.\n\nFORMATO (exato):\nLinha 1: TITULO: <título reescrito, até 65 caracteres>\nLinha 2: RESUMO: <1-2 frases>\nDepois: corpo em HTML puro (começando com <p>).`;
...
function montar(art, img, fonte) {
  const alt = (art.titulo || "inteligência artificial").replace(/"/g, "");
  ...
  const font = `\n<div style="background:#fafafa;border:1px solid #eee;border-radius:12px;padding:14px 18px;margin:26px 0;font-size:.9em;color:#555"><strong>Fonte e atualidade:</strong> matéria baseada no noticiário recente sobre IA${fonte ? " (como " + fonte + ")" : ""}. Confira sempre a fonte oficial para detalhes.</div>`;
  const aviso = `\n<p style="font-size:.85em;color:#888;border-top:1px solid #eee;padding-top:14px;margin-top:26px"><em>Conteúdo informativo sobre tecnologia. Ferramentas e recursos de IA mudam rápido.</em></p>`;
...
(async () => {
  ...
  console.log("manchetes novas disponíveis:", noticias.length, "| meta desta run:", PER_RUN);
  let token = await getToken();
  let ok = 0;
  for (let i = 0; i < noticias.length && ok < PER_RUN; i++) {
    const n = noticias[i];
    const art = await gerar(n);
    if (!art || jaTem(art.titulo)) continue;
    const seed = Math.floor(Math.random() * 999999);
    const img = IMG_BASE ? `${IMG_BASE}/${slugify(art.titulo)}.webp?q=${encodeURIComponent("artificial intelligence technology futuristic neon")}&s=${seed}` : `https://image.pollinations.ai/prompt/${encodeURIComponent("artificial intelligence futuristic")}?width=1200&height=630&seed=${seed}&nologo=true`;
    const html = montar(art, img, n.fonte);
    const label = CATS[Math.floor(Math.random() * CATS.length)];
    let done = false;
  ...
  console.log(`\n=== ${ok} matérias publicadas nesta run ===`);
